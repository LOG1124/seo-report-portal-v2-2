...
### Update v2.3 from GitHub

First complete the guide’s Windows GitHub update procedure. It must stop if the Git source, staging content, or existing v2.3 Skill directory contains `private/`. Keep the Git source outside the installed Skill directory. Verify that this installed script exists before continuing:

```text
%USERPROFILE%\.codex\skills\seo-report-portal-v2-3\scripts\import_google_archive.py
```
